...
const envSchema = base.extend(runtime.shape).refine((data) => {
  // If not running tests, require real-service credentials/URIs
  if (data.NODE_ENV === 'test') return true;

  const hasMongo = typeof data.MONGO_URI === 'string' && data.MONGO_URI.length > 0;
  const hasJwt = typeof data.JWT_SECRET === 'string' && data.JWT_SECRET.length >= 32;
